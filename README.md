...
- Tables and LaTeX formulas laid out for narrow 58mm thermal paper, with formula strokes and table lines thickened so thin symbols (fractions, `=`, absolute-value bars) stay readable.
- Insert images (rendered in grayscale for thermal paper).
...
- Links and bare URLs
- Tables (auto column widths tuned for narrow paper)
- Basic HTML
- LaTeX formulas, including absolute values `|x|` inside table cells
- Images

Mermaid, complex diagrams, and theme CSS are not fully supported yet.
...
Third-party library notices are listed in [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md).
...
- 连接已配对的 P1 并打印。
- 调整字号、浓度和尾纸。
- 表格和 LaTeX 公式按 58mm 窄热敏纸排版，公式笔画和表格线做了加粗，让分数线、`=`、绝对值竖线等细笔画在热敏纸上也清楚。
- 插入图片（转灰度适配热敏纸）。
...
- 表格（按内容自动分配列宽，适配窄纸）
- 基础 HTML
- LaTeX 公式，包括表格单元格里的绝对值 `|x|`
- 图片

暂时没有完整支持 Mermaid、复杂图表和主题 CSS。
...
第三方开源库说明见 [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md)。
